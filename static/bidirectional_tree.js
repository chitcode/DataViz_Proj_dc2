...
//Variables
var margin = {'top': 10, 'right': 5, 'bottom': 0, 'left': 30};
// This will be the maximum dimensions
chartWidth = (960 - margin.right - margin.left);
chartHeight = (500 - margin.top - margin.bottom);
centralHeight = chartHeight / 2;
centralWidth = chartWidth / 2;
linkLength = 100;
duration = 200;
...
var treeChart = function(d3Object) {
  ...
  this.directions = ['leftward', 'rightward'];
};
...
drawChart = function() {
  ...
  this.treeData = {};
  var self = this;
  d3.json('http://localhost:8000/testing.json', function(error, allData) {
...
treeChart.prototype.graphTree = function(config) {
  ...
  function update(source, originalData, g) {
    ...
    var forLeftward = direction == 'leftward';
    var node_class = direction + 'Node';
    var link_class = direction + 'Link';
    var rightSign = (leftward) ? -1 : 1;
    var nodeColor = (leftward) ? '#37592b' : '#8b4513';
    ...
    nodes.forEach(function(d) {
      d.y = rightwardSign * (d.width * linkLength) + config.centralWidth;
      d.x = d.x - offsetX;
      // Position for origin node.
	  if (d.name == 'origin') {
        d.x = config.centralWidth;
        d.y += rightwardSign * 25;
      }
    });
    ...
    nodeEnter.append('text')
        .attr('x', function(d) {
          return forLeftward ? -10 : 10;})
        .attr('dy', '.35em')
        .attr('text-anchor', function(d) {
          return forLeftward ? 'end' : 'start';})
        .text(function(d) {
